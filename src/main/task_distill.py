...
def main():
    ...
    if args.quant:
        from transformer.quant_bert_modeling import (
            TinyBertForSequenceClassification as student_bert,
        )
    else:
    ...
    if "roberta" in args.teacher_model:
        tokenizer = AutoTokenizer.from_pretrained(
            "roberta-base", do_lower_case=args.do_lower_case
        )
    else:
    ...
    if not args.do_eval:
        if "roberta" in args.teacher_model:
            teacher_model = TinyRobertaForSequenceClassification.from_pretrained(
    ...
    if "roberta" in args.student_model:
        student_model = TinyRobertaForSequenceClassification.from_pretrained(
            args.student_model,
            num_labels=num_labels,
        ...
        if args.ablation_init:
            student_model = student_bert.from_scratch(
                args.student_model,
            ...
            student_model = student_bert.from_pretrained(
                args.student_model,
                hidden_act=args.hidden_act,
                softmax_act=args.softmax_act,
                num_labels=num_labels,
