...
class WikiProcessor(DataProcessor):
    """Processor for the Wikitext data set."""
...
def convert_examples_to_features(examples, max_seq_length,
    ...
    for i in range(len(examples)):
        encoded_inputs = tokenizer.encode(
            examples[i], return_tensors="pt", max_length=max_seq_length, truncation=True, padding='max_length'
        )
...
def get_tensor_data(data):
    ...
    all_label_ids = torch.tensor([label for label in labels], dtype=torch.long)
    all_input_ids = torch.tensor([f for f in feats], dtype=torch.long)
...
def do_eval(model, task_name, eval_dataloader,
            device, output_mode, eval_labels, tokenizer):
    eval_loss = 0
    nb_eval_steps = 0
    preds = []
    ...
    for batch_ in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if output_mode == "classification":
            ...
            loss_fct = CrossEntropyLoss()
            tmp_eval_loss = loss_fct(logits.view(-1, logits.size(-1)), label_ids.view(-1))
            # loss = torch.sum(
    ...
    result = compute_metrics(task_name, preds, eval_labels.numpy(), tokenizer)
...
def main():
    ...
    default_params = {
        ...
        "wiki": {"num_train_epochs": 30, "max_seq_length": 512},
    }
    ...
    tokenizer = GPT2Tokenizer.from_pretrained(args.student_model, do_lower_case=args.do_lower_case)
    tokenizer.pad_token_id = tokenizer.eos_token_id

    if not args.do_eval:
        if not args.aug_train:
            train_examples = [text for text in train_dataset["text"] if text is not None and text != ""]
        else:
    ...
    eval_examples = [text for text in eval_dataset["text"] if text is not None and text != ""]
    ...
    if not args.do_eval:
        teacher_model = GPT2LMHeadModel.from_pretrained(args.teacher_model, activation_function="gelu_new", softmax_act="softmax")
        teacher_model.to(device)
        # set teacher model to eval mode
        #teacher_model.eval()
    
    result_t = do_eval(teacher_model, task_name, eval_dataloader, device, output_mode, eval_labels, tokenizer)
    ...
    """
    if args.ablation_init:
        student_model = student_gpt2.from_scratch(args.student_model, activation_function=args.hidden_act, softmax_act=args.softmax_act, fit_size=teacher_model.config.hidden_size)
        logger.info("student load from scratch")
    else:
        student_model = student_gpt2.from_pretrained(args.student_model, activation_function=args.hidden_act, softmax_act=args.softmax_act, fit_size=teacher_model.config.hidden_size)
